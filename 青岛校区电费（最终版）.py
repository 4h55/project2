...
smtp_port = 587  # 或使用端口号465，但需要SSL
username = "2798951371@qq.com"
password = "mdhfkwciwhvgdegh"
msg = MIMEMultipart()
msg['from']=username
msg['To']=input("您的邮箱地址")
...
def job():
    ...
    # 使用示例：
        ...
        toast('未来一段时间内不用担心断电问题啦', '请安心学习',
              image=r'"C:\Users\4h55\Downloads\微信图片_20241030223931.png"',
              duration='long',
              )
    #发送qq邮件提醒
    if float(elc[0]) < a:
        msg['Subject']="警告！电费不足，请尽快充值" # 邮件主题
        body =f"电量仅剩{float(elc[0])}度"

    else:
        msg['Subject'] = "未来一段时间内不用担心断电问题啦"  # 邮件主题
        body = "请安心学习"
    msg.attach(MIMEText(body, 'plain'))  # 添加文本内容到邮件中
    server = smtplib.SMTP(smtp_server, smtp_port)
    server.starttls()  # 启用TLS加密连接
    server.login(username, password)  # 使用授权码登录
    server.sendmail(msg['From'], msg['To'], msg.as_string())  # 发送邮件，as_string()将MIME对象转换为字符串格式
